  ...
  return (
    ...
    >
      ...
      <div class={width}>
        ...
        <div class="stagger-grid space-y-0 divide-y divide-gray-100">
          {items.map((item, idx) => {
            ...
            const inner = (
              <div class={`group flex items-center gap-6 py-6 transition-all duration-300 hover:pl-2 ${isDark ? '' : ''}`}>
                ...
                <div class="flex-1">
                  ...
                  {item.description && (
                    <div class={`mt-1 text-sm leading-relaxed ${isDark ? 'text-gray-400' : 'text-slate-500'}`} dangerouslySetInnerHTML={{ __html: item.description }} />
                  )}
                </div>
            ...
            return (item as any).url
              ? <a href={urlFn((item as any).url)} class="block hover:no-underline">{inner}</a>
              : <div>{inner}</div>;
          })}
        </div>
      </div>
